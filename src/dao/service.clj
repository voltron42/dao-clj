...
(defn inquire [db query-spec & et-al]
  (let [[args & {:keys [opts arg-spec table-schema] :or {opts {} arg-spec {} table-schema {}}}] (if (even? (count et-al)) (into [{}] et-al) et-al)
        query-args (c/build-query query-spec args arg-spec table-schema)]
    (jdbc/query db query-args opts)))

(defn build-inquisitor [db query-spec & {:keys [opts arg-spec table-schema]
                                         :or {opts {} arg-spec {} table-schema {}}}]
...
(defn build-inquisitor-for-func [db query-func & {:keys [opts arg-spec table-schema] :or {opts {} arg-spec {} table-schema {}}}]
...
(defn build-dao [{:keys [name columns insert get update delete] :as dao-spec}]
  ...
  (merge {}
         (if-not (nil? insert)
           {:create (let [inserters (reduce-kv #(assoc %1 %2 (c/build-insert-factory %3)) {} insert)]
                      (fn [label args]
           ...
           {})
         (if-not (nil? update)
           {:update (let [updaters (reduce-kv #(assoc %1 %2 (c/build-update-factory %3)) {} update)]
                      (fn [label args]
           ...
           {})
         (if-not (nil? delete)
           {:delete (let [deleters (reduce-kv #(assoc %1 %2 (c/build-delete-factory %3)) {} delete)]
                      (fn [label args]
           ...
           {})
         (if-not (nil? get)
           {:get (let [getters (reduce-kv #(assoc %1 %2 (c/build-get-factory %3)) {} get)]
                   (fn [label args]
